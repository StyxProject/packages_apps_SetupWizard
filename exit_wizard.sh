...
adb shell pm enable co.styxproject.setupwizard/co.styxproject.setupwizard.SetupWizardExitActivity || true
if [[ ! -z "$has_google_suw" ]]
then
    wait ${!}
...
sleep 1
adb shell am start co.styxproject.setupwizard/co.styxproject.setupwizard.SetupWizardExitActivity || true
if [[ ! -z "$has_google_suw" ]]
then
    wait ${!}
    sleep 1
